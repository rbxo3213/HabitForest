"use client";

import { useState } from "react";
import { auth, db } from "@/lib/firebase";
import { createUserWithEmailAndPassword, signInWithEmailAndPassword } from "firebase/auth";
import { doc, setDoc } from "firebase/firestore";
import { useRouter } from "next/navigation";
import { Loader2 } from "lucide-react";

export default function SeedPage() {
  const [status, setStatus] = useState("대기 중...");
  const [isLoading, setIsLoading] = useState(false);
  const router = useRouter();

  const handleSeed = async () => {
    setIsLoading(true);
    setStatus("테스트 유저 4명 생성 시작...");

    const usersToCreate = [
      { email: "test1@test.com", pass: "123123", name: "테스터1" },
      { email: "test2@test.com", pass: "123123", name: "테스터2" },
      { email: "test3@test.com", pass: "123123", name: "테스터3" },
      { email: "test4@test.com", pass: "123123", name: "테스터4" },
    ];

    try {
      const uids = [];
      
      // Create users
      for (const u of usersToCreate) {
        setStatus(`유저 생성 중: ${u.email}...`);
        try {
          const cred = await createUserWithEmailAndPassword(auth, u.email, u.pass);
          uids.push(cred.user.uid);
          
          // Save to Firestore
          await setDoc(doc(db, "users", cred.user.uid), {
            uid: cred.user.uid,
            email: u.email,
            nickname: u.name,
            themeColor: "bg-pink-400",
            groupId: "test-group-1",
            createdAt: new Date().toISOString()
          });
        } catch (err: any) {
          // If already exists, just sign in to get UID
          if (err.code === "auth/email-already-in-use") {
            const cred = await signInWithEmailAndPassword(auth, u.email, u.pass);
            uids.push(cred.user.uid);
          } else {
            throw err;
          }
        }
      }

      setStatus("그룹(마을) 데이터 생성 중...");
      
      // Create a test group
      await setDoc(doc(db, "groups", "test-group-1"), {
        id: "test-group-1",
        name: "테스트 마을",
        members: uids,
        level: 1,
        points: 0,
        theme: "기본"
      });

      setStatus("완료! test1@test.com으로 로그인합니다...");
      
      // Sign in as test1 to finish
      await signInWithEmailAndPassword(auth, "test1@test.com", "123123");
      
      setTimeout(() => {
        router.push("/dashboard");
      }, 1000);

    } catch (error: any) {
      console.error(error);
      setStatus(`에러 발생: ${error.message}`);
    } finally {
      setIsLoading(false);
    }
  };

  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-gray-50">
      <div className="bg-white p-8 rounded-3xl shadow-lg text-center max-w-sm w-full space-y-6">
        <h1 className="text-2xl font-bold">DB 초기화 (테스트용)</h1>
        <p className="text-gray-500 text-sm break-keep">
          테스트 유저 4명(test1~4@test.com)을 생성하고 Firestore에 마을 데이터를 구성합니다.
        </p>
        
        <div className="bg-gray-100 p-4 rounded-xl text-sm font-medium text-gray-700 break-all">
          {status}
        </div>

        <button
          onClick={handleSeed}
          disabled={isLoading}
          className="w-full bg-black text-white rounded-xl py-3 font-medium hover:bg-gray-800 disabled:opacity-50 transition-colors flex justify-center items-center gap-2"
        >
          {isLoading && <Loader2 size={18} className="animate-spin" />}
          유저 4명 자동 생성하기
        </button>
      </div>
    </div>
  );
}
